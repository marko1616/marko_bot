...
class HistoryManager:
    def __init__(self, init_history: Optional[list[dict[str, str]]] = []) -> None:
        ...
        self.model_sim.load_state_dict(torch.load(
            r'../../AI/history_attention/history_attn.pth', map_location=config.default_device))
        self.model_sim.eval()
    ...
    def add_to_history(self, input_text: str, model_reply: str) -> None:
        ...
        self.history.append(
            {"role": Role.ASSISTANT.value, "content": model_reply})
    ...
    def _get_sim(self, input_text: str, history_text: list[str]) -> list:
        ...
        history_text_embedded = torch.Tensor(
            [self.get_embedding(text) for text in history_text])
