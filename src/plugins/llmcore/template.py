...
def _get_jinja_template(template: "Template", tokenizer: "PreTrainedTokenizer") -> str:
    ...
    if isinstance(template, Llama2Template):
        ...
        jinja_template += "{% set content = " + \
            system_message + " + message['content'] %}"
        jinja_template += "{% endif %}"
    jinja_template += "{% if message['role'] == 'user' %}"
    user_message = _convert_slots_to_jinja(
        template.format_user.apply(), tokenizer)
...
_register_template(
    name="baichuan",
    format_user=StringFormatter(
        slots=["<reserved_102>{{content}}<reserved_103>"]),
    efficient_eos=True,
)
...
_register_template(
    name="openchat",
    format_user=StringFormatter(slots=["GPT4 Correct User: {{content}}", {
                                "eos_token"}, "GPT4 Correct Assistant:"]),
    format_assistant=StringFormatter(slots=["{{content}}", {"eos_token"}]),
    format_system=StringFormatter(slots=[{"bos_token"}, "{{content}}"]),
    force_system=True,
)
