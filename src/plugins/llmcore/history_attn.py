...
class HistoryAttention(nn.Module):
    ...
    def forward(self, current_input, historical_inputs):
        ...
        cosine_similarity = F.cosine_similarity(
            current_input.unsqueeze(1), historical_inputs, dim=-1)

        # 计算注意力分数
        # 需要在query上增加一个维度，使其shape变为(batch_size, 1, d_q)
        # 以便与key进行矩阵乘法
        attention_scores = torch.matmul(query.unsqueeze(
            1), key.transpose(-2, -1))  # shape: (batch_size, 1, his_num)
        # 防止softmax梯度消失
        attention_scores /= torch.sqrt(torch.tensor(self.d_k,
                                       dtype=query.dtype))
        # 加权余弦相似度
        ...
        attention_output = attention_scores.squeeze(
            1)  # shape: (batch_size, his_num)
