...
class ChatAgent():
    def __init__(self,
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(
            config.mode_loading_param["pretrained_model_name_or_path"], use_fast=False, trust_remote_code=config.mode_loading_param["trust_remote_code"])

        assert config.quantization_mode in ["4bit", "8bit", None]
        quantization_config = quantization_configs[config.quantization_mode]

        config.mode_loading_param["quantization_config"] = quantization_config
        if config.device_map:
            config.mode_loading_param["device_map"] = config.device_map
        self.model = AutoModelForCausalLM.from_pretrained(
            **config.mode_loading_param)
        
        if config.model_lora_path:
            if isinstance(config.model_lora_path, str):
                self.model = PeftModel.from_pretrained(
                    self.model, config.model_lora_path)
                self.model.merge_adapter()
            elif isinstance(config.model_lora_path, list):
                for index, path in enumerate(config.model_lora_path):
                    self.model = PeftModel.from_pretrained(
                        self.model, path, adapter_name=f"lora_{index}")
    ...
    def stream_chat(self, input_text: str) -> Iterator[str]:
        ...
        input_ids = torch.IntTensor(self.build_prompt(input_text)[0]).to(
            self.config.default_device).unsqueeze(0)
        generate_input = {
            "input_ids": input_ids,
            "do_sample": True,
            "top_k": self.top_k,
        ...
        token_list = []
        for _ in range(self.config.max_gen_len):
